...
export default class QrScanner {
    ...
    start() {
        ...
        if (window.location.protocol !== "https:") console.warn("The camera stream is only readable if the page is transferred via https.");
        this._active = true;
    ...
    static scanImage(imageOrFileOrUrl, sourceRect = null, worker = null, canvas = null, fixedCanvasSize = false, alsoTryWithoutSourceRect = false) {
        ...
        if (sourceRect && alsoTryWithoutSourceRect) return promise.catch(() => QrScanner.scanImage(imageOrFileOrUrl, null, worker, canvas, fixedCanvasSize));
        else return promise
    }
